...
def loadDog(pos, orn):
    ...
    quadruped = p.loadURDF("a1/urdf/a1.urdf",[pos[0],pos[1],0.48],[0,0,0,1], flags = urdfFlags,useFixedBase=False)
...
def getState(goal_x, goal_y, quadruped):
...
def getEpsReward(goal_x, goal_y, eps, jointIds, quadruped, Horizon):
    ...
    for h in range(Horizon):
        ...
        if h == (Horizon-1):
            futureS = start
            futureE = end
            endDist = getState(goal_x, goal_y, quadruped).tolist()[6]
        else:
            futureS = end
        ...
        if h == 2:
            startDist = getState(goal_x, goal_y, quadruped).tolist()[6]
...
def main():
    ...
    goal_x = np.random.randint(low=0, high=10)
    goal_y = np.random.randint(low=0, high=10)


    maxForceId, quadruped, jointIds = loadDog(pos, orn)
    ...
    for iter in range(Iterations):
        ...
        for e in range(Epochs): 
            ...
            for eps in range(currEpsNum):
                ...
                # get cost of episode
                cost = getEpsReward(goal_x, goal_y, episode, jointIds, quadruped, Horizon)
